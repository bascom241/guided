...
import { emailContent,resetPasswordEmailContent,resetSuccessfulEmailContent } from "./emailTemplate";
dotenv.config();

const sendEmail = async (to:any,verificationToken:any,fullName:any) => {
  try {
...
const sendPasswordResetEmail = async(to:any,fullName:any, resetLink:any) => {
    try {
        // Initialize Brevo API
        let defaultClient = SibApiV3Sdk.ApiClient.instance;
        let apiKey = defaultClient.authentications["api-key"];
...
const sendResetSuccessfullEmail = async (to:any) =>{
    try {
        // Initialize Brevo API
        let defaultClient = SibApiV3Sdk.ApiClient.instance;
        let apiKey = defaultClient.authentications["api-key"];
